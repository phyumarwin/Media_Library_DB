...
if ($total_items > 0) {
    ...
    $total_pages = ceil($total_items / $items_per_page); //ceil() is change to nearest integer

    // limit results in redirect
    $limit_results = "";
    if (!empty($search)) {
        $limit_results = "s=" . urlencode(htmlspecialchars($search)) . "&";  //& is use to separate multiple query parameter
    ...
    // filtering data by search and category
    if (!empty($search)) {
